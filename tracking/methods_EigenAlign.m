function [tracks_EigenAlign, EigenAlign_runtime] = methods_EigenAlign(source, target, cnt, tracks_EigenAlign, EigenAlign_runtime, run_local)
if ~run_local
    path_1 = '/storage/coda1/p-hl94/0/schaudhary9/rich_project_pchbe2/OptimalTransport/other_methods/EigenAlign_spectral-graph-alignment-master/spectral-graph-alignment-master';
else
    path_1 = 'D:\Shivesh\OptimalTransport\tracking\other_methods_QAP\EigenAlign_spectral-graph-alignment-master\spectral-graph-alignment-master';
end
...
map=ones(size(adj,1),size(adj_src,1));

% EigenAlign
sol = EigenAlign(adj,adj_src,map,gamma);
sol = sol{1,1};
...
tracks_EigenAlign{1,cnt} = C;
EigenAlign_runtime = EigenAlign_runtime + toc;
