...
function [track_link_accuracy,MT,ML,frags,length_1_frags,frags_per_gt,mean_track_purity,mean_obj_purity] = quantify_accuracy_charles(base_track_gt, frame_track_gt, allCenters)
    ...
    %%% match track hypothesis objs to gt objects
    track_ids = [];
    track_TPs = [];
    track_left_TP_ids = {};
    for i = 1:size(allCenters,2)
        curr_centers = allCenters{1,i};
        track_centroid = reshape([curr_centers.Centroid],3,[])';
        if size(track_centroid,1) < size(curr_centers,2)
            error('empty centroids exist')
        end
        curr_frame_gt = frame_track_gt{1,i};
        frame_gt_centroid = curr_frame_gt(:,2:4);
        match_dist = repmat(diag(track_centroid*track_centroid'),1,size(frame_gt_centroid,1)) + repmat(diag(frame_gt_centroid*frame_gt_centroid')',size(track_centroid,1),1) - 2*track_centroid*frame_gt_centroid';
        [sort_dist,sort_index] = sort(match_dist,2,'ascend');
        ids = zeros(size(track_centroid,1),1);
        TP = zeros(size(track_centroid,1),1);
        for n = 1:size(track_centroid,1)
            ids(n,1) = curr_frame_gt(sort_index(n,1),1);
            TP(n,1) = curr_frame_gt(sort_index(n,1),5);
        end
        track_ids = [track_ids,ids];
        track_TPs = [track_TPs,TP];

        all_TP_ids = curr_frame_gt(curr_frame_gt(:,5) == 1,1);
        left_TP_ids = [];
        for n = 1:size(all_TP_ids,1)
            if isempty(find(all_TP_ids(n,1) == ids))
                left_TP_ids = [left_TP_ids;all_TP_ids(n,1)];
            end
        end
        track_left_TP_ids{1,i} = left_TP_ids;
    end
            
    all_track_hyps = struct();
    cnt = 1;
    for n = 1:size(track_ids,1)
        track_hyp = [];
        track_hyp_frame = [];
        track_hyp_only_TP = [];
        for t = 1:size(track_ids(n,:),2)
            curr_frame_gt = frame_track_gt{1,t};
            if track_ids(n,t) == 0 % hypothesis in curr t does not match to any obj in frame_track_gt (empty centroid)
            else
                track_hyp = [track_hyp,track_ids(n,t)];
                track_hyp_frame = [track_hyp_frame,t];
                if curr_frame_gt(find(track_ids(n,t) == curr_frame_gt(:,1)),5) == 1
                    track_hyp_only_TP = [track_hyp_only_TP,track_ids(n,t)];
                else
