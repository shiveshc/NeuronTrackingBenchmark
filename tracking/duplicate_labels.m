function node_label = duplicate_labels(curr_labels,X,Y,Z,PA_matrix,LR_matrix,DV_matrix,clamped_neurons)
    ...
    for i = 1:size(curr_labels,1)
        if node_label(i,2) == 1
            node_label(i,3) = consistency_score(i,node_label(i,1),node_label,X,Y,Z,PA_matrix,LR_matrix,DV_matrix);
        end
    end
...
%     visualize_duplicates(thisimage_r,mu,node_label,Neuron_head)
    ...
    for i = 1:size(uniq_label,1)
        ...
        if size(curr_label_index,1) > 1
            ...
            clamped_neuron_present = clamped_neurons(find(ismember(clamped_neurons,curr_label_index)),:);
...
function score = consistency_score(i,i_label,node_label,X,Y,Z,PA_matrix,LR_matrix,DV_matrix)
    ...
    score = (PA_consistency_score + LR_consistency_score + DV_consistency_score)/(3*max_consistency_score);
end