function writeJobs2Txt_multi( myfunction, input_P, input_R )
...
for p = 1:size(input_P,2)
    for r = 1:size(input_R,2)
        inp_dir = ['D:\Shivesh\OptimalTransport\tracking\Results\Other_methods'];
        fileList = dir(inp_dir);
        sim_number = [];
        for i = 1:size(fileList,1)
            name = fileList(i).name;
            if ~isdir([inp_dir,'\',name])
                underscore_pos = strfind(name,'_');
                current_P = str2num(name(1,underscore_pos(1,1)+1:underscore_pos(1,2)-1));
                current_R = str2num(name(1,underscore_pos(1,2)+1:underscore_pos(1,end)-1));
                if and(current_P == input_P(1,p), current_R == input_R(1,r))
        ...
        file = fopen(['jobs_gweot_alltoone_',num2str(input_P(1,p)),'_',num2str(input_R(1,r)),'.txt'],'w');
        total_jobs = 0;
        cnt = 0;
        ...
        for k = begin:50 % 100 repeats for each fixed random selected landmarks
            call = ['matlab -nodisplay -singleCompThread -r ', '"', myfunction, '(', num2str(input_P(1,p)), ',',num2str(input_R(1,r)),',',num2str(k), ')"'];
        ...
        file = fopen(['batch_process_gweot_alltoone_',num2str(input_P(1,p)),'_',num2str(input_R(1,r)),'.sh'], 'w');
        fprintf(file, '#!/bin/bash\n');

        for i = 1: num_batches - 1
                call = ['qsub -vBATCHSIZE=',num2str(batch_size), ',BATCHNUM=',num2str(i-1),[' paralleljob_gweot_alltoone_',num2str(input_P(1,p)),'_',num2str(input_R(1,r)),'.pbs']];
                %'\r' is only needed if reading the file with microsoft notepad:
                %fprintf(file, '%s\r\n', call);
                % the following should be fine for any linux systems:
                fprintf(file, '%s\n', call);
        ...
        call = ['qsub -vBATCHSIZE=',num2str(remaining_jobs), ',BATCHNUM=',num2str(i),[' paralleljob_gweot_alltoone_',num2str(input_P(1,p)),'_',num2str(input_R(1,r)),'.pbs']];
        fprintf(file, '%s\n', call);
        fclose(file);
        
        
        %%%% write paralleljob file
        file = fopen(['paralleljob_gweot_alltoone_',num2str(input_P(1,p)),'_',num2str(input_R(1,r)),'.pbs'], 'w');
        N = [num2str(input_P(1,p)),'_',num2str(input_R(1,r)),'_gweot_alltoone'];
        account = 'GT-hl94-joe';
        q = 'inferno';
        walltime = '24:00:00';
        JOBFILE = ['jobs_gweot_alltoone_',num2str(input_P(1,p)),'_',num2str(input_R(1,r)),'.txt'];
