...
function [results_struct] = quantify_accuracy_cinda_v2(base_track_gt, frame_track_gt, trajectories, ids)
    ...
    for n = 1:size(trajectories,1)
        ...
        curr_track = trajectories{n,1};
        curr_track_length = size(curr_track,1);
        for t = 1:curr_track_length
            curr_id_indx = find(curr_track(t,1) == ids(:,3));
            curr_track_frame = ids(curr_id_indx,2);
            curr_frame_gt = frame_track_gt{1,curr_track_frame};
            if curr_track(t,1) == 0 % zeros indicate frames where particles do not exist because of temporary particle disappearance
            else
                curr_track_particle_id = ids(curr_id_indx,1);
                track_hyp = [track_hyp,curr_track_particle_id];
                track_hyp_frame = [track_hyp_frame,curr_track_frame];
                if curr_frame_gt(find(curr_frame_gt(:,1) == curr_track_particle_id),5) == 1
    ...
    per_frame_track_link_accuracy = frame_summary(:,2)./frame_summary(:,1);
    track_link_accuracy = mean(per_frame_track_link_accuracy);
    
    % obj linking accuracy
    obj_summary = [];
    count_frame = 0;
    correct = 0;
    for n = 1:size(gt_obj_hyp_map,1)
    ...
    %               if and(sum(gt_obj_hyp_map(n,1:t-1)) == 0, all_track_hyps(curr_track_hyp).track_hyp_frame(1,1) == t) % this means curr t is the first timepoint when track start for the gt object
        ...
        obj_summary = [obj_summary;count_frame,correct];
    end
    obj_link_accuracy = obj_summary(:,2)./obj_summary(:,1);
    mean_obj_link_accuracy = mean(obj_link_accuracy);
    ...
    results_struct = add_var_to_struct(results_struct,track_link_accuracy,...
                                                        MT,...
                                                        ML,...
                                                        frags,...
                                                        length_1_frags,...
                                                        ...
                                                        track_purity,...
                                                        obj_purity,...
                                                        per_frame_track_link_accuracy,...
                                                        obj_link_accuracy,...
                                                        mean_obj_link_accuracy,...
                                                        all_track_hyps,...
                                                        gt_obj_hyp_map);
end