...
function [results_struct] = quantify_accuracy_alltoone_v2(base_track_gt, frame_track_gt, tracks)
    ...
    for t = 1:size(tracks,2)
        ...
        for n = 1:size(curr_frame_gt,1)
            ...
            if ~flag
                ...
                curr_track = tracks{1,t};
                prev_match = curr_track(n,1);
                
                if prev_match == 0 % this means obj hyp is not assigned any label
                else
                    for next_t = t+1:size(tracks,2)
                        ...
                        if isempty(next_match)
                            ...
                            prev_match = next_track(next_match,1);
                        end
                    end
                end
