...
function compare_methods_synthetic_tracking_CRF_alltorand(precision,recall,num)
...
%             frame_to_match = base_track_gt{};
            ...
            for i = 1:size(frame_track_gt,2)
                ...
                source = source(:,2:4);
                [PA_matrix,LR_matrix,DV_matrix] = make_pos_relation_mat(source);

                target = frame_track_gt{1,i};
                target = target(:,2:4);

                [node_label, lambda_PA, lambda_LR, lambda_DV, lambda_angle, loc_sigma] = CRF_track(PA_matrix, LR_matrix, DV_matrix, source, target, [1,1,1,0,1]);
                node_label = node_label(:,1);
                
                % handle duplicate matches
                uniq_match = unique(node_label);
                for n = 1:size(uniq_match,1)
                    curr_match = uniq_match(n,1);
                    curr_match_index = find(node_label == curr_match);
                    ...
                    if size(curr_match_index,1) > 1
                        ...
                        node_label(curr_match_index(sort_index(2:end,:),:),:) = 0;
                    end
                end
                
                tracks{1,i} = node_label;
            end
            curr_runtime = toc;
...
%             results = [results;num,precision(1,p),recall(1,r),precision_param,recall_param,lambda_PA,lambda_LR,lambda_DV,lambda_angle,loc_sigma,noise_level,curr_runtime,track_link_accuracy,MT,ML,frags,length_1_frags,frags_per_gt,mean_track_purity,mean_obj_purity];
            
            results_temp = quantify_accuracy_alltoone_v2(base_track_gt, frame_track_gt, tracks);
            results_temp = add_var_to_struct(results_temp,precision(1,p),recall(1,r),precision_param,recall_param,lambda_PA,lambda_LR,lambda_DV,lambda_angle,loc_sigma,noise_level,curr_runtime);
            if isempty(fieldnames(results)); results = results_temp; else; results = [results;results_temp]; end
            
            save(['/storage/coda1/p-hl94/0/schaudhary9/rich_project_pchbe2/OptimalTransport/Results/OM_alltorand/results_CRF_alltorand_',num2str(precision(1,p)),'_',num2str(recall(1,r)),'_',num2str(num),'.mat'],'results')
