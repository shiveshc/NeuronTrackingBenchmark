function writeJobs2Txt_data_for_CNMF_poisson( myfunction, input_pht_cnt, input_background_int )
    ...
    for r = 1:size(input_background_int,2)
        ...
        for i = 1:size(fileList,1)
            ...
            if and(~strcmp('.',name), ~strcmp('..',name))
                ...
                if and(current_speckle_noise == input_pht_cnt(1,p), current_background_int == input_background_int(1,r))
        ...
        file = fopen(['jobs_CNMF_poisson_data_',num2str(input_pht_cnt(1,p)),'_',num2str(input_background_int(1,r)),'.txt'],'w');
        total_jobs = 0;
        cnt = 0;
        ...
        for k = 2:2 % 100 repeats for each fixed random selected landmarks
            call = ['matlab -nodisplay -singleCompThread -r ', '"', myfunction, '(', num2str(input_pht_cnt(1,p)), ',',num2str(input_background_int(1,r)),',',num2str(k), ')"'];
        ...
        file = fopen(['batch_process_CNMF_poisson_data_',num2str(input_pht_cnt(1,p)),'_',num2str(input_background_int(1,r)),'.sh'], 'w');
        fprintf(file, '#!/bin/bash\n');
        
        if num_batches == 1
                call = ['qsub -vBATCHSIZE=',num2str(batch_size), ',BATCHNUM=',num2str(0),[' paralleljob_CNMF_poisson_data_',num2str(input_pht_cnt(1,p)),'_',num2str(input_background_int(1,r)),'.pbs']];
                %'\r' is only needed if reading the file with microsoft notepad:
                %fprintf(file, '%s\r\n', call);
                % the following should be fine for any linux systems:
                fprintf(file, '%s\n', call);
            ...
            for i = 1: num_batches - 1
                    call = ['qsub -vBATCHSIZE=',num2str(batch_size), ',BATCHNUM=',num2str(i-1),[' paralleljob_CNMF_poisson_data_',num2str(input_pht_cnt(1,p)),'_',num2str(input_background_int(1,r)),'.pbs']];
                    %'\r' is only needed if reading the file with microsoft notepad:
                    %fprintf(file, '%s\r\n', call);
                    % the following should be fine for any linux systems:
                    fprintf(file, '%s\n', call);
            ...
            call = ['qsub -vBATCHSIZE=',num2str(remaining_jobs), ',BATCHNUM=',num2str(i),[' paralleljob_CNMF_poisson_data_',num2str(input_pht_cnt(1,p)),'_',num2str(input_background_int(1,r)),'.pbs']];
            fprintf(file, '%s\n', call);
        end
        fclose(file);
        ...
        file = fopen(['paralleljob_CNMF_poisson_data_',num2str(input_pht_cnt(1,p)),'_',num2str(input_background_int(1,r)),'.pbs'], 'w');
        N = [num2str(input_pht_cnt(1,p)),'_',num2str(input_background_int(1,r)),'_CNMF_poisson_data'];
        account = 'GT-hl94-joe';
        q = 'inferno';
        walltime = '96:00:00';
        JOBFILE = ['jobs_CNMF_poisson_data_',num2str(input_pht_cnt(1,p)),'_',num2str(input_background_int(1,r)),'.txt'];
