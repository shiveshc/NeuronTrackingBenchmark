...
function create_synthetic_data_3d_for_CNMF_poisson(pht_cnt, background_intensity, num)
rng shuffle
rng shuffle
...
%     curr_frame_with_noise = make_poisson_noise_img(curr_frame_with_background, pht_cnt, 'uint16');
%     curr_frame_with_noise_norm = mat2gray(curr_frame_with_noise);
    noisy_vid(:,:,:,t) = curr_frame_with_background;
    gt_vid(:,:,:,t) = curr_frame_with_background;
end
[gt_vid, noisy_vid] = make_poisson_noise_vid(gt_vid, pht_cnt, 'uint16');
disp('videos created')
...
save_name = ['/storage/coda1/p-hl94/0/schaudhary9/rich_project_pchbe2/OptimalTransport/data_for_CNMF_poisson_v2/tp_1000/vid_',num2str(pht_cnt),'_',num2str(background_intensity),'_',num2str(num)];
if exist(save_name,'dir')
else
    mkdir(save_name)
    disp(['save dir created - ', save name])
end 
